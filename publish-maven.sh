...
KOTLIN_VERSION_LIST=""
# 支持 ohos 的 Kotlin 版本列表（从 gradle.properties 读取，可通过 -ok 参数覆盖）
...
# 跨端模块目录名
MODULE_KMP="KuiklyCamera"

# Android 原生模块目录名
MODULE_ANDROID="KuiklyCameraAndroid"

# Android 模块发布任务（afterEvaluate + singleVariant("release")）
ANDROID_PUBLISH_TASK="publishReleasePublicationToMavenRepository"
...
if [ -f "${GRADLE_PROPS}" ]; then
  ...
  # 从 gradle.properties 读取 OHOS_KOTLIN_VERSION_LIST
  ...
  # 从 gradle.properties 读取 GROUP_ID
  ...
  # 从 gradle.properties 读取 MAVEN_REPO_URL（仅在命令行和环境变量都未指定时）
  if [ -z "${MAVEN_URL}" ]; then
  ...
  # 从 gradle.properties 读取 MAVEN_USERNAME（仅在环境变量未指定时）
  ...
  # 从 gradle.properties 读取 MAVEN_PASSWORD（仅在环境变量未指定时）
...
function usage() {
  echo ""
  echo " 发布 KuiklyCamera 产物到 Maven 仓库"
  echo ""
  ...
  echo "  -k, --kotlin        指定编译的 Kotlin 版本列表, 使用 , 分割, 默认读取 gradle.properties 中 KOTLIN_VERSION_LIST"
  echo "  -ok, --ohos-kotlin  指定鸿蒙 Kotlin 版本列表, 使用 , 分割, 默认读取 gradle.properties 中 OHOS_KOTLIN_VERSION_LIST"
  echo "  -s, --snapshot      指定是否 SNAPSHOT (true/false)"
  echo "  -l, --local-repo    指定是否发布到本地 repo (true/false)"
  echo "  -m, --maven-url     指定 Maven 仓库地址, 默认读取 gradle.properties 中 MAVEN_REPO_URL"
  echo "  -c, --clean         指定是否需要 clean 后编译 (true/false)"
  echo "  -d, --debug         是否开启 Gradle debug 模式 (true/false)"
  echo "  -h, --help          查看帮助"
  echo ""
  echo " 环境变量 (优先级高于 gradle.properties，低于命令行参数):"
  echo "  mavenUrl            Maven 仓库地址"
  echo "  mavenUserName       Maven 认证用户名"
  echo "  mavenPassword       Maven 认证密码"
  echo ""
}
...
function publishModule() {
  ...
  local core_version
  core_version=$(get_kuikly_core_version "${kt_version}" "${enable_ohos}")
  ...
  # 通过环境变量传递版本号给 build.gradle.kts
  export kuiklyBizVersion="${build_version}"

  log_info "发布 ${project} (kotlin=${kt_version}, version=${build_version}, ohos=${enable_ohos})"
  echo "> [build] ${GRADLEW} :${project}:${task} ${settings_flag} -PkotlinVersion=${kt_version} -PkuiklyCoreVersion=${core_version} -PHARMONY_ENABLED=${harmony_flag} -PmavenVersion=${build_version} ${common_args}"
...
function publishAll() {
  ...
  for kt_version in "${VERSIONS[@]}"; do
    ...
    # 发布 Android 原生模块 (KuiklyCameraAndroid)
    if ! publishModule "${MODULE_ANDROID}" "${ANDROID_PUBLISH_TASK}" "${kt_version}" "${build_version}" "false"; then
      return 1
    fi
